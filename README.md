...
| 阶段 | 能力 |
|---|---|
| D | 地图选址（Leaflet + OpenStreetMap，无需 key），点选落标记 + 反向地理编码 |
...
app/
  ...
  jobs/page.tsx, client.tsx     职位列表 + AI 聊天
...
components/job/JobCard.tsx         职位卡片（Prompt F）
...
lib/geocode.ts                位置→城市/suburb 匹配（Prompt C）
lib/llm.ts                    大模型客户端（OpenAI 兼容）
lib/mock.ts                   YEEYI 不可用时的本地示例数据
store/mapStore.ts, jobStore.ts, chatStore.ts   Zustand 状态
hooks/useLocation.ts          读取位置 hook
types/                        公共类型（yeeyi/job/geo/suburb）
...
> **坐标说明**：`data/suburb-coords.json` 为 suburb 的邮编级近似中心点（100% 覆盖），供 `nearestSuburbs()` 按坐标就近匹配；来源为社区开源数据集 [joelkoen/postcodes-au](https://github.com/joelkoen/postcodes-au)（CC），缺失邮编用 OSM Nominatim 兜底。地图选点默认走该就近匹配。
